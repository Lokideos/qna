...
describe Ability do
  ...
  describe 'for user' do
    ...
    let(:question_rating) { create :rating, value: 1, ratable: create(:question, user: other), user: user }
    let(:answer_rating) { create :rating, value: 1, ratable: create(:answer, user: other),     user: user }
    let(:bad_question_rating) { create :rating, value: 1, ratable: create(:question, user: user), user: other}
    let(:bad_answer_rating) { create :rating, value: 1, ratable: create(:answer, user: user),     user: other}
    ...
    it { should be_able_to :rate_good, question_rating.ratable, user: other }
    it { should be_able_to :rate_good, answer_rating.ratable,   user: other }
    it { should be_able_to :rate_bad, question_rating.ratable,  user: other }
    it { should be_able_to :rate_bad, answer_rating.ratable,    user: other }
